...
function Edit(props) {
  ...
  /* input의 value속성에 값을 설정하면 React는 readOnly속성으로 렌더링한다.
    따라서 이 값을 수정하려면 반드시 스테이트가 필요하다. onChange이벤트리스너에 setter함수를
    호출해서 값을 변경할 수 있다. */
  // input의 갯수만큼 스테이트를 실행한다.
  ...
  return (<>
    ...
    <article>
      ...
      <form onSubmit={(event) => {
        ...
        // 경우에 따라서 목록으로 이동할 수 있다.
        // navigate('/list');
      }}>
        {/* 수정의 경우 게시물의 일련번호를 서버로 전송해야 하므로 아래와 같이 hidden타입의
          상자를 만들어서 값을 설정해야 한다. hidden타입은 수정의 대상이 아니므로 onChange
          리스너는 필요하지 않다. */}
        ...
        <table id="boardTable">
          <tbody>
            <tr>
              <td>작성자</td>
              {/* 스테이트에 저장된 값을 value에 설정하고, onChange 이벤트 리스너를 통해
                입력한 값을 실시간으로 변경해서 적용한다. */}
