...
export default function RealtimeCRUD() {
  ...
  /* 데이터 쓰기 -> set() : 기본 쓰기 작업에 사용. 지정된 참조에 데이터를 저장하고 해당 경로의
      기존 데이터를 모두 변경할 수 있다. */
  const writeUserData = (userId, userName, userPass) => {
    // 새로운 게시물 등록을 위한 Key값을 생성한다. -o3yga.. 이런식으로
    const newPostKey = push(child(ref(realtime), 'tempValue')).key;

    /* 최상위 노드를 users로 하고 하위는 사용자가 입력한 ID를 데이터로 구분하여 입력한다.
  ...
  // 데이터 수정
  const editUserData = (userId, userName, userPass) => {
    // 고유키 생성
    const newPostKey = push(child(ref(realtime), 'tempValue')).key;
    // 수정할 데이터를 객체 형식으로 작성
    const postData = {
  ...
  let adder = '-' + addNum;
  const id = 'rowin1102' + adder;
  const name = '김영진' + adder;
