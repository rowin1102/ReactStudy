...
const GlobalTop = (props) => {
  console.log('#Life', 'GlobalTop ==> 1. 컴포넌트 실행(함수 호출)');
  ...
  useEffect(function () {
    ...
    fetch('./json/myData.json')
      ...
      /* 요청에 성공하면 json파일의 데이터가 매개변수를 통해 반환된다.
        반환되는 데이터는 Text형식이므로 JSON포맷으로 변환 후 반환한다. */
      return result.json();
    })
    .then((json)=>{
  ...
  for(var i=0; i<myList.length; i++) {
    ...
    listTag.push(
      <li key={data.id}>
        {/* data-id 속성에 설정된 값은 Event객체의 target속성 하윙의 dataset.id를 통해
          읽어올 수 있다. 이 속석에 게시물의 일련번호인 num을 설정하고 있다. */}
...
function App() {
  // dto.json의 내용을 저장할 스테이트 생성. 초기값은 빈객체로 생성.
  var [myResult, setMyResult] = useState({});

  /* 클릭시 내부에 저장된 dto.json파일을 get방식으로 요청한 후 콜백데이터를 받아오는 기능의
    함수를 프롭스로 전달한다. 자식 컴포넌트는 이 함수를 호출할때 게시물의 일련번호를 인수로
    전달한다. */
