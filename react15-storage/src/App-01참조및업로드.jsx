...
export default function App() {
  ...
  /* ref()를 호출할때 경로를 두번째 인수로 전달하여 경로 트리에서 하위 위치를
    가르키는 참조객체를 만들 수 있다. */
  // 경로 : root/images 
  const imagesRef1 = ref(storage, 'images');
  const imagesRef2 = ref(storage, 'images/myFile.jpg');

  /* parent 및 root 속성을 사용해서 한단계 상위로 이동하거나 최상위의 경로를 참조할 수 있다. */
  const imagesRef3 = imagesRef2.parent;
  const imagesRef4 = imagesRef2.root;

  /* fullPath, name 등의 속성으로 참조를 조사하여, 이것이 가리키는 파일을 자세히 파악할 수 있다. */
  ...
  return (<>
    ...
    <input type="file" name="myfile" onChange={e => {
      ...
      uploadBytes(imageRef, e.target.files[0])
        .then(snapshot => console.log('업로드성공', snapshot));
    }} />
  </>); 
}