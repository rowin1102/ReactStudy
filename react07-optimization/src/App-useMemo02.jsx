...
function App() {
  ...
  /* Step2 : 상수를 객체로 변경. JS에서는 객체를 선언할때마다 새로운 참조값을 할당하게 된다.
      즉, 새로운 렌더링을 위해 App컴포넌트가 호출될때마다 참조값이 변경된다.
      따라서 useEffect()가 지속적으로 호출된다. 즉, 불필요한 실행이 발생되게 된다. */
  // const switchMode =  { 
  ...
  /* Step1 : 기본(원시) 타입의 값을 의존성 배열에 추가했으므로, 값의 변화가 있을때만
      useEffect가 재실행된다.
    Step2 : 객체형으로 변경하면 App컴포넌트가 렌더링될때다마 새로운 참조값이 할당되므로 값이
      변경된 것으로 인식하여 useEffect가 지속적으로 호출되는 문제가 생긴다. 즉, 애플리케이션의
      성능이 저하될 수 있다.
    Step3 : App컴포넌트가 렌더링될때마다 지속적으로 참조값이 변경되는 부분을 차단하기 위해
      useMemo를 통해 Memoization한 값을 사용하도록 코드를 수정하면 switchMode가 변경될때만
      useEffect를 재실행한다. */
  useEffect(() => {
