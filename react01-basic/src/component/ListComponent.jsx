...
function ListComponent(props) {
  ...
  return (<>
  {/* JSX를 통해 UI를 표현할때 최상위 엘리먼트는 반드시 하나여야 한다.
    현 시점에는 React에서 프리그먼트(Fragment)를 제공하므로 이를 통해
    묶어주면 된다. 프레그먼트는 빈 꺽쇄괄호로 표현하면 된다. */}
  <header>
    <h2>게시판-목록</h2>
  </header>
  <nav>
    {/* a태그의 기본동작(화면이동)을 차단하고, 프롭스로 전달받은 함수를
      호출하여 스테이트를 write로 변경한다. 즉, 작성 화면으로 전환하기 위한 링크를
      생성한다. */}
  ...
  <article>
    <table id="boardTable">
      ...
      <tbody>
        {/* 스타일 설정을 위해 사용한 class 속성은 className으로 변경해야 한다.
          JS에서는 이미 class 키워드가 있기 때문이다. */}
        <tr>
